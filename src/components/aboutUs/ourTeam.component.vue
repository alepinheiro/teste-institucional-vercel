...
<script lang="ts" setup>
  ...
  function horizontalLoop(
    ...
    const snap: (v: number) => number =
      //@ts-expect-error verificar type error
    ...
    function toIndex(index: number, vars: typeof tl.vars = {}) {
    ...
    tl.next = (vars: typeof tl.vars) => toIndex(curIndex + 1, vars);
    tl.previous = (vars: typeof tl.vars) => toIndex(curIndex - 1, vars);
    tl.current = () => curIndex;
    tl.toIndex = (index: number, vars: typeof tl.vars) => toIndex(index, vars);
    tl.times = times;
    tl.progress(1, true).progress(0, true);
    if (!config.reversed || !tl.vars || !tl.vars.onReverseComplete) return;
