  ...
  <section class="py-20">
    <div
      ...
      <div class="flex flex-col gap-6 sm:gap-8 flex-1">
        ...
        <p class="text-textPrimary w-5/6 md:w-full md:text-justify sm:w-full text-base sm:text-justify">
        ...
        <a :href="`https://simulador.bext.vc/${useObjectToQueryString(route.query)}`" target="_blank"
          class="font-bold text-white bg-primary rounded-xl px-4 py-2 w-fit sm:hidden hover:-translate-y-1 hover:shadow-md hover:shadow-black/50 transition-all active:scale-90">Simule
          on-line de graça
        </a>
      </div>
      <div class="w-full flex-1 flex flex-col gap-2">
        <div class="flex md:mx-auto mt-20 min-md:pt-0 h-128 w-full ">
          <div class="flex flex-row justify-around w-full h-full items-end relative">
            <span class="absolute left-0 -top-10 min-md:-top-5 w-1/3 sm:w-1/2 text-primary">
              Veja e compare as <b>taxas médias</b> do mercado:
            </span>
            <div v-for="{ id, tax, title } of products" :key="id"
              ...
              <div :style="{ height: tax * 10 + '%' }" :class="[id === 1 ? 'bg-primary' : 'bg-[#D9D9D9]']"
                class="w-full rounded-t-lg flex-shrink-0 hover:bg-primary transition-all ease-out duration-700"></div>
              <div class="absolute -mb-14 h-10 text-center">
                <span class="text-sm min-md:whitespace-nowrap group-hover:font-bold transition">
                  {{ title }}
                </span>
              </div>
            </div>
        ...
        <span class="text-xs text-right sm:text-center w-full">
