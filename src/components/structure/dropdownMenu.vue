<template>
  <div class="relative" @mouseenter="showPopOver = true" @mouseleave="showPopOver = false">
    <div class="text-white decoration">Para você</div>
    <div v-if="showPopOver" class="absolute z-10 w-48 p-2 bg-white rounded shadow-lg">
      ...
      <div :class="linkStyle">
      <router-link :to="'/emprestimo-com-garantia-de-veiculo/BestCar'+this.$root.utms" class="text-textPrimary hover:text-complementaryColor1 text-sm" >Empréstimo com garantia de veículo</router-link>
      </div>
    </div>
  </div>
</template>
